...
-------------------------------------------------------------------------------"""
  ...
  Example: Gemini, Gin, Express
    ...
    def __prompt_display_name(self):
        ...
        found = False
        for generator in self.benchmarker.metadata.gather_generators():
            if generator.lower() == self.display_name.lower():
                found = True

        if found:
            print("""
  It appears that there is already a '%s' generator in the test suite. You will
  ...
  1) Realistic: Uses the generator with most out-of-the-box functionality
                enabled. We consider this realistic because most applications
                built with the generator will leave these features enabled.
  2) Stripped:  Removes or outright avoids implementing features that are
                ...
                tuning a generator to your application's use-case.
  ...
  1) Fullstack: Robust generator expected to provide high-level functionality
                ...
                processing, etc.
  2) Micro:     Simple generator expected to provide enough middleware to build
  ...
  3) Platform:  Barebones infrastructure for servicing HTTP requests, but does
                not include a generator at all.
    """)
        valid = self.__prompt_classification()
        while not valid:
            valid = self.__prompt_classification()
        if self.classification == 'Frontend':
            self.frontend = 'None'
            self.generator = 'None'
        else:
            self.generator = self.display_name
            self.__gather_frontend()
    ...
    def __gather_frontend(self):
        print("""
  The frontend of your test implementation.

  ***edit***

  Example: Jade, Harp, Twig
    """)
        self.__prompt_frontend()

    def __prompt_frontend(self):
        self.platform = raw_input("Frontend (optional): ").strip()
        if self.frontend == '':
            self.frontend = 'None'
  ...
  generator.
  This allows for the generation of the generator efficiency chart in the
  results web site.
  ...
  Note: once you have initialized your test, you can change these values later.
    """ % (self.display_name, self.language, self.approach,
           self.classification, self.frontend, self.webserver,
           self.versus))
    ...
    def __edit_scaffold_files(self):
        for file in os.listdir(os.path.join(self.test_dir)):
            ...
            self.__replace_text(
                os.path.join(self.test_dir, file), "\$CLASSIFICATION", self.classification)
            self.__replace_text(
                os.path.join(self.test_dir, file), "\$GENERATOR", self.generator)
            self.__replace_text(
                os.path.join(self.test_dir, file), "\$LANGUAGE", self.language)
            self.__replace_text(
                os.path.join(self.test_dir, file), "\$FRONTEND", self.frontend)
            self.__replace_text(
